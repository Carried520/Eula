...
public class SqlCommands : InteractionModuleBase<SocketInteractionContext>
{
    ...
    public SqlCommands(ApplicationDbContext context)
    {
        _context = context;
    }
    ...
    {
        await DeferAsync();
        UserEntity? query = await _context.Users.FirstOrDefaultAsync(x => x.Id == Context.User.Id);
        if (query != null)
        {
        ...
        await FollowupAsync($"Inserted/Updated {entries} entries ");
    }
    ...
    {
        await DeferAsync();
        List<UserEntity> query = await _context.Users.ToListAsync();
